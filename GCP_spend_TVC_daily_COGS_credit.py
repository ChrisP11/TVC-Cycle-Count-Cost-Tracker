#########################################################################################
###  This script gets the credits per day for TVC COGS from the BigQuery Billing DB   ###
#########################################################################################
...
def query_bq():
    ...
    # not checking for null query results - if nothing is returned, no update occurs to the DB
    # same for if the date is not already in the DB table
    for row in results:
        q = """ Update CostTVCcogsPerDay
                Set Credit_per_day = %.2f
                Where uDate = '%s' """ % (row.credit, row.Date)
        logger.info("The credit applied is %s, to date: %s ", row.credit, row.Date)
